...
LEFT JOIN 
    sys.database_role_members AS rm ON rm.member_principal_id = princ.principal_id
LEFT JOIN 
    sys.database_principals AS roles ON rm.role_principal_id = roles.principal_id
LEFT JOIN 
    sys.objects AS obj ON perm.major_id = obj.object_id
WHERE 
    princ.name LIKE '%app_user%' 
    AND obj.name IS NOT NULL
ORDER BY 
    PrincipalName, ObjectName;
